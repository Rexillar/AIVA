...
const Canvas = () => {
  ...
  const drawGrid = useCallback((canvas) => {
    if (!canvas || !showGrid || !canvas.getContext() || !canvas.width || !canvas.height) return;
    ...
    if (gridRef.current) {
      try {
        ...
        // Grid might be from a different canvas instance
  ...
  const zoomIn = useCallback(() => {
    ...
    const newZoom = Math.min(4, currentZoom * 1.25);
  ...
  useEffect(() => {
    ...
    canvas.on('mouse:down', (opt) => {
      ...
      // Enable panning with Alt+drag or pan tool
      if ((activeTool === 'select' && opt.e.altKey) || activeTool === 'pan') {
        isPanning = true;
    ...
    const handleResize = () => {
      if (parentElement && fabricCanvas.current && !fabricCanvas.current.disposed) {
        const newWidth = parentElement.offsetWidth || window.innerWidth - 320;
        const newHeight = parentElement.offsetHeight || window.innerHeight - 64;
  ...
  const saveToHistory = useCallback(async () => {
    ...
    try {
      ...
      addToHistory(currentStateString);
    } catch (error) {
...
// Layer/Z-index management functions
  ...
  const updateConnectionVisual = useCallback((connection) => {
    ...
    const fromPoint = {
      x: fromShape.left + fromShape.width / 2,
      y: fromShape.top + fromShape.height / 2
    };
    const toPoint = {
      x: toShape.left + toShape.width / 2,
      y: toShape.top + toShape.height / 2
    };

    // Create arrow line
    const arrow = new fabric.Path(`M ${fromPoint.x} ${fromPoint.y} L ${toPoint.x} ${toPoint.y}`, {
      stroke: '#007bff',
      strokeWidth: 2,
      fill: '',
      selectable: false,
    ...
    if (connection.mode === 'directional') {
      ...
      const arrowhead = new fabric.Triangle({
        ...
        height: 10,
        fill: '#007bff',
        left: toPoint.x - 5 * Math.cos(angle),
        top: toPoint.y - 5 * Math.sin(angle),
      ...
      const angle1 = Math.atan2(toPoint.y - fromPoint.y, toPoint.x - fromPoint.x);
      const angle2 = Math.atan2(fromPoint.y - toPoint.y, fromPoint.x - toPoint.x);
      
      const arrowhead1 = new fabric.Triangle({
        width: 8,
        height: 8,
        fill: '#007bff',
        left: toPoint.x - 4 * Math.cos(angle1),
        top: toPoint.y - 4 * Math.sin(angle1),
      ...
      const arrowhead2 = new fabric.Triangle({
        ...
        height: 8,
        fill: '#007bff',
        left: fromPoint.x - 4 * Math.cos(angle2),
        top: fromPoint.y - 4 * Math.sin(angle2),
  ...
  fabricCanvas.current.off('selection:created');
    ...
    switch (activeTool) {
      ...
      case 'pen':
        // Ensure brush is initialized before enabling drawing mode
        ...
        break;
      case 'highlighter':
        // Highlighter is like pen but with different opacity
        if (!fabricCanvas.current.freeDrawingBrush) {
          fabricCanvas.current.freeDrawingBrush = new fabric.PencilBrush(fabricCanvas.current);
        }
        ...
        const handleConnectMouseDown = (options) => {
          ...
          if (target && target.type === 'group' && target.metadata && target.metadata.type !== 'frame' && target.metadata.type !== 'connection') {
            if (!connectionStart) {
              ...
              // visual highlight
              try { target.set('stroke', '#007bff'); target.set('strokeWidth', 3); } catch (e) { console.warn('Could not highlight target', e); }
              fabricCanvas.current.renderAll();
            } else if (connectionStart !== target) {
              createPersistentConnection(connectionStart, target, connectionMode);

              // Reset highlight on start
              try { connectionStart.set('stroke', '#000000'); connectionStart.set('strokeWidth', 2); } catch (e) { console.warn('Could not reset highlight', e); }
  ...
  const shapeCategories = {
    flowchart: [
      ...
      { id: 'diamond', icon: FaGem, label: 'Decision', action: () => setSelectedShape('diamond') },
      { id: 'parallelogram', icon: FaSquare, label: 'Data', action: () => setSelectedShape('parallelogram') },
      { id: 'cylinder', icon: FaSquare, label: 'Database', action: () => setSelectedShape('cylinder') },
      { id: 'document', icon: FaSquare, label: 'Document', action: () => setSelectedShape('document') },
    ],
    containers: [
      { id: 'rounded-rect', icon: FaSquare, label: 'Container', action: () => setSelectedShape('rounded-rect') },
      { id: 'hexagon', icon: FaSquare, label: 'Hexagon', action: () => setSelectedShape('hexagon') },
      { id: 'octagon', icon: FaSquare, label: 'Octagon', action: () => setSelectedShape('octagon') },
      { id: 'cloud', icon: FaSquare, label: 'Cloud', action: () => setSelectedShape('cloud') },
    ],
    symbols: [
      ...
      { id: 'triangle', icon: FaSquare, label: 'Triangle', action: () => setSelectedShape('triangle') },
      { id: 'pentagon', icon: FaSquare, label: 'Pentagon', action: () => setSelectedShape('pentagon') },
    ],
    connectors: [
      ...
      <div className="w-16 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col items-center py-4 space-y-2">
        {tools.map((tool) => (
          ...
          >
            ...
            <div className="absolute left-full ml-3 px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 whitespace-nowrap z-10 shadow-xl">
        ...
        {showShapeMenu && (
          <div className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl overflow-hidden z-50 animate-fadeIn w-64 max-w-[90vw]">
            <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-gray-900 dark:to-gray-800 px-4 py-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              <div>
                <h3 className="text-sm font-semibold text-gray-800 dark:text-white">Shapes</h3>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Click to insert on canvas</p>
              </div>
              <button
            ...
            <div className="p-3 max-h-[500px] overflow-y-auto">
              {/* Flowchart Shapes */}
              <div className="mb-4">
                <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 px-1">Flowchart</div>
                <div className="grid grid-cols-2 gap-2">
                  {shapeCategories.flowchart.map((shape) => (
                    <button
                      key={shape.id}
                      draggable="true"
                      onDragStart={(e) => {
                      ...
                      className={`group relative w-full aspect-square rounded-lg flex flex-col items-center justify-center transition-all duration-200 ${
                    ...
                    >
                      <div className="w-6 h-6 mb-1 flex items-center justify-center">
                        {typeof shape.icon === 'function' ? shape.icon() : <shape.icon className={`transition-transform ${selectedShape === shape.id ? '' : 'group-hover:scale-110'}`} />}
                      </div>
              ...
              {/* Container Shapes */}
              <div className="mb-4">
                <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 px-1">Containers</div>
                <div className="grid grid-cols-2 gap-2">
                  {shapeCategories.containers.map((shape) => (
                    <button
                      key={shape.id}
                      draggable="true"
                      onDragStart={(e) => {
                      ...
                      className={`group relative w-full aspect-square rounded-lg flex flex-col items-center justify-center transition-all duration-200 ${
                        selectedShape === shape.id
                          ...
                          : 'bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-400 hover:bg-green-50 dark:hover:bg-green-900/20 hover:text-green-600 dark:hover:text-green-400 hover:shadow-md hover:scale-105'
                      }`}
                      title={shape.label}
                    >
                      <div className="w-6 h-6 mb-1 flex items-center justify-center">
              ...
              {/* Symbol Shapes */}
              <div className="mb-4">
                <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 px-1">Symbols</div>
                <div className="grid grid-cols-2 gap-2">
                  {shapeCategories.symbols.map((shape) => (
                    <button
                      key={shape.id}
                      draggable="true"
                      onDragStart={(e) => {
                      ...
                      className={`group relative w-full aspect-square rounded-lg flex flex-col items-center justify-center transition-all duration-200 ${
                        selectedShape === shape.id
                          ...
                          : 'bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 hover:text-purple-600 dark:hover:text-purple-400 hover:shadow-md hover:scale-105'
                      }`}
                      title={shape.label}
                    >
                      <div className="w-6 h-6 mb-1 flex items-center justify-center">
              ...
              {/* Connector Shapes */}
              <div>
                <div className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2 px-1">Connectors</div>
                <div className="grid grid-cols-2 gap-2">
                  {shapeCategories.connectors.map((shape) => (
                    <button
                      key={shape.id}
                      draggable="true"
                      onDragStart={(e) => {
                      ...
                      className={`group relative w-full aspect-square rounded-lg flex flex-col items-center justify-center transition-all duration-200 ${
                        selectedShape === shape.id
                          ...
                          : 'bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 hover:text-orange-600 dark:hover:text-orange-400 hover:shadow-md hover:scale-105'
                      }`}
                      title={shape.label}
                    >
                      <div className="w-6 h-6 mb-1 flex items-center justify-center">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 px-4 py-2 border-t border-gray-200 dark:border-gray-700">
              <p className="text-xs text-gray-500 dark:text-gray-400 text-center">Drag shapes to canvas or click to select</p>
            </div>
          </div>
        )}
      </div>
          ...
          {canvases.map((canvasTab) => (
            <div key={canvasTab._id} className="flex items-center">
              <button
                ...
                className={`px-3 py-1 text-sm rounded-t-md border-t border-l border-r flex items-center space-x-2 ${
                  activeCanvasId === canvasTab._id
